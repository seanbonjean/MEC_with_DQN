"""
  ...
  eval_net用于预测q_eval,这个神经网络拥有最新的神经网络参数
...
class DeepQNetwork:
    ...
    def __init__(
        ...
        self.replace_target_iter = replace_target_iter  # # 更换target_net的步数（多少步更换一次）
    ...
    # --------------------------------------------------创建两个网络-----------------------------------------
    # 预测Q估计的神经网络具备最新的参数eval
    # 而预测Q现实的神经网络使用的参数则是很久以前的target.
    def _build_net(self):
        # ------------------ 创建eval神经网络（预测Q估计），及时更新参数------------------
        self.s = tf.placeholder(tf.float32, [None, self.n_features], name='s')  # 用于接收观测值
        self.q_target = tf.placeholder(tf.float32, [None, self.n_actions], name='Q_target')  # 用来接受q_target的值
        with tf.variable_scope('eval_net'):
            # c_names(collections_names)在更新 target_net 参数时会用到
        ...
        # Q现实时候的预测值是下一个状态的（s_）
        # ------------------创建 target（预测Q现实）神经网络, 提供 target Q ------------------
    ...
    def store_transition(self, s, a, r, s_):
        ...
        # 用新内存替换旧内存 总memory的大小是固定的, 如果超出总大小, 旧memory就被新memory替换
        index = self.memory_counter % self.memory_size
        self.memory[index, :] = transition  # 替换的过程
